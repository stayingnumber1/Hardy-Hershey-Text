...
def draw_svg_text(char, face, offset, vertoffset, parent):
    ...
    if Debug:
        inkex.debug("Char: " + str(char) + " PathString: " + str(pathString) + str(
            " Midpoint: " + str(midpoint) + " SplitString0: " + str(splitString[0]) + " SplitString1: " + str(
                splitString[1]) + "\n"))
...
def draw_stroke(xpa, ypa, endx, endy, parent):
    ...
    style = {'stroke': '#000000', 'fill': 'none', 'stroke-width': '0.5', 'stroke-miterlimit': '4', }
    text_attribs = {'style': simplestyle.formatStyle(style),
                    'd': 'M ' + str(xpa) + ' ' + str(ypa) + ' L ' + str(endx) + ' ' + str(endy)}
    inkex.etree.SubElement(parent, inkex.addNS('path', 'svg'), text_attribs)
...
def handleLayoutTitle(layout, layout2, sel_layout, textlines,
                      cur_layer):  # Place Text on its Coordinates and selected Alignment on matching with selected Layout
...
def handleCoord(ifset, coord, sizex, sizey, textlines,
                cur_layer):  # Watch for seperate Alignment and Text Manipulation of eatch Text Coordinates
    xpa = coord.getAttribute(
        "x")  # X Position of Text-Field (depending on text alignment / left = left-mid position of Text / right = right-mid position of text / center = mid position of text
    ypa = coord.getAttribute(
        "y")  # Y Position of Text-Field (depending on text alignment / left = left-mid position of Text / right = right-mid position of text / center = mid position of text
    ...
    if val_coord == 'Line 1':
        ...
        startPlacerLoop(cur_layer, textline1, textline2, sizex, sizey, xpa, ypa, align, tsize, textf, sbtwl, vco, vcp,
                        hcp, mrg)  # Place Looped Text
        textline = False
    else:
    ...
    if textline:  # Check if Textline is available and if its defined to be placed
        if not ifset or textlines[checknr] is not "":
            placeLayoutetText(cur_layer, textline, sizex, sizey, xpa, ypa, align, tsize, textf, sbtwl, vco, vcp, hcp,
                              mrg)  # Place Text
    if Debug:
        inkex.debug(str("Koordinaten: " + val_coord) + str(" Ausrichtung: " + align + "\n"))


def startPlacerLoop(cur_layer, textline1, textline2, sizex, sizey, xpa, ypa, align, tsize, textf, sbtwl, vco, vcp, hcp,
                    mrg):
...
def placeLayoutetText(cur_layer, textline, sizex, sizey, xpa, ypa, align, tsize, textf, sbtwl, vco, vcp, hcp, mrg):
    ...
    if tsize != "":  # Resize Text if tsize / Text Size / is defined
        ...
        if textf != "standard":
            vcp += float(
                tsize) / mmpcpx1  # Divide wished Font-Size with its mean height value (in mm) ( Add Values if Compressing is enabled)
            hcp += float(tsize) / mmpcpx1
        else:
            vcp += float(
                tsize) / mmpcpx2  # Divide wished Font-Size with its mean height value (in mm) ( Add Values if Compressing is enabled)
    ...
    tlc = 'translate(' + str(xpa) + ',' + str(ypa) + ') ' + 'scale(' + str(vcp) + ', ' + str(
        hcp) + ')'  # Top Left Corner
...
class Hershey(inkex.Effect):
    ...
    def effect(self):
        ...
        g_attribs = {inkex.addNS('label', 'svg'): 'Hershey'}
        # Create a node instance  parent_node: self.current+layer
        ...
        # spacing1: distance between letters default=0
        if self.options.spacing1 != "":
            spacing1 = float(self.options.spacing1)
        else:
            spacing1 = 0

        # spacing2: distance between the Glyph-Table default=3
        ...
        t = 'translate(' + str(self.view_center[0] - w * compressx / 2) + ',' + str(self.view_center[1]) + ')' ')'
        g.set('transform', t)
